function S = assemble_density_with_fun(A,T,Nd,parallel)
%ASSEMBLEGLOBALBILINEARFORM Summary of this function goes here
%   Detailed explanation goes here

if parallel
    S = assemble_density_with_fun_parallel(A,T,Nd);
else
    S = assemble_density_with_fun_sequential(A,T,Nd);
end
end
...
function S = assemble_density_with_fun_sequential(A,T,Nd)
...
for k = 1:size(T,1)
    ...
    for i = 1:3
        for j = i:3
            int = diag((phi_in_quad(:,i)'.*A_in_quad)'*(phi_in_quad(:,j)'.*A_in_quad));
...
function S = assemble_density_with_fun_parallel(A,T,Nd)
